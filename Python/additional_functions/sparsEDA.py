        ...
        if explicitA:
            if len(np.array([R_I]).flatten()) == 1:
                p = scipy.linalg.solve(np.array(R_I).reshape(-1, 1),
                                       np.matmul(R[:, activeSet].transpose(), R[:, newIndex]), transposed=True,
                                       lower=False)
            else:
                p = scipy.linalg.solve(R_I, np.matmul(R[:, activeSet].transpose(), R[:, newIndex]), transposed=True,
                                       lower=False)
...
def lasso(R, s, sr, maxIters, epsilon):
    ...
    while done == 0:
        ...
        if len(np.array([R_I]).flatten()) == 1:
            z = scipy.linalg.solve(R_I.reshape([-1, 1]), np.sign(c[np.array(activeSet).flatten()].reshape(-1, 1)),
                                   transposed=True, lower=False)
        else:
            z = scipy.linalg.solve(R_I, np.sign(c[np.array(activeSet).flatten()].reshape(-1, 1)), transposed=True,
                                   lower=False)

        if len(np.array([R_I]).flatten()) == 1:
            dx[np.array(activeSet).flatten()] = scipy.linalg.solve(R_I.reshape([-1, 1]), z, transposed=False,
                                                                   lower=False)
        else:
        ...
        if ((lmbda - gammaMin) < OptTol) or ((lmbdaStop > 0) and (lmbda <= lmbdaStop)) or (
                (epsilon > 0) and (LA.norm(res) <= epsilon)):
            newIndices = []
            removeIndices = []
            done = 1
...
# %%
# This method provide the low component and the driver
# response of a galvanic skin response signal longer than 70 seconds.

# signalIn - galvanic skin response
# sr - samlple rate
# epsilon - step reminder
# Kmax - maximum number of LARS iterations
# dmin - maximum distance between sparse reactions
# rho - minimum threshold of sparse reactions

# outpus:
#       - driver - driver responses, tonic component
#       - SCL - low component.
#       - MSE - reminer of the signal fitting
